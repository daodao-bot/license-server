...
import editForm from "@/views/product/form.vue";
import { useProduct } from "@/views/product/hook";

import { Order, PageParam } from "@/api";
import { ProductData, ProductSearch, ProductUpsert } from "@/api/product";
import { productSearch, productUpsert } from "@/api/product";
...
const form = reactive<ProductUpsert>({
  id: null,
  code: null,
  name: null,
  valid: null
...
const formRef = ref();
const tableRef = ref();
const dataList: Ref<ProductData[]> = ref([]);
...
const onSearch = async () => {
  loading.value = true;
  const searchParam: ProductSearch = toRaw(form);
  const param: PageParam<ProductSearch> = {
...
const onExport = async () => {
  ...
  let page = 1;
  let size = 1000;
  const pageParam: PageParam<ProductSearch> = {
    page: page,
    size: size,
    param: {
      ...searchParam
  ...
  await doExport(productSearch, pageParam);
};

const openDialog = (title = "新增", row?: ProductData) => {
  addDialog({
    title: `${title}数据`,
    props: {
      formInline: {
    ...
    beforeSure: (done, { options }) => {
      ...
      const curData = options.props.formInline as ProductData;

      function chores() {
        message(`您${title}了 code = ${curData.code} 的这条数据`, {
          type: "success"
        });
        done(); // 关闭弹框
        onSearch(); // 刷新表格数据
      ...
      FormRef.validate((valid: any) => {
        if (valid) {
          ...
          if (title === "新增") {
            // 实际开发先调用新增接口，再进行下面操作
            const upsertParam: ProductUpsert = emptyToNull({
              id: null,
              ...curData
            }) as ProductUpsert;
            productUpsert({ param: upsertParam })
              .then(({ data }) => {
